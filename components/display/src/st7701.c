...
// 显示参数（取自 NanoSoul-Alpha bsp_board_pins.h，常量内联到此处）。
#define LCD_DSI_LANES       1     // WLK2802 是 1-lane 模块
#define LCD_DSI_LANE_MBPS   700   // 480x640 RGB565@60Hz 约需 ~530Mbps，留余量
#define LCD_DPI_CLK_MHZ     25
#define LCD_RST_GPIO        GPIO_NUM_NC   // FPC 上 RC 复位，无 GPIO
#define MIPI_DPHY_LDO_CHAN  3             // LDO_VO3：DSI/CSI 共用的 MIPI DPHY 轨
#define MIPI_DPHY_LDO_MV    2500
...
esp_err_t st7701_init(void)
{
    ...
    esp_lcd_dsi_bus_config_t bus_config = {
        ...
        .lane_bit_rate_mbps = LCD_DSI_LANE_MBPS,
    };
    ESP_RETURN_ON_ERROR(esp_lcd_new_dsi_bus(&bus_config, &s_dsi_bus), TAG, "dsi bus");
    ESP_LOGI(TAG, "DSI bus OK (%d lanes @ %d Mbps)", LCD_DSI_LANES, LCD_DSI_LANE_MBPS);
     ...
     * on Waveshare's reference ST7701 panels. @25 MHz pclk gives ~61 Hz refresh.
     * ST7701 accepts a wide porch tolerance.
     */
    ...
    ESP_LOGI(TAG, "ST7701S init OK, fb0=%p fb1=%p, %dx%d RGB565",
             s_fbs[0], s_fbs[1], DISP_H_RES, DISP_V_RES);
    return ESP_OK;
}
