...
> **预览（当前 62×72 板）**：[`../pcb/output/final62_top.png`](../pcb/output/final62_top.png) / [`../pcb/output/final62_bottom.png`](../pcb/output/final62_bottom.png)。（`nanosoul_routed.png` / `nanosoul_dimensions.pdf` 为旧大板历史预览。）
> **板上已印接线丝印**：底部条带印了 J1/J2/J8 逐脚 + 电机脚序图例，顶部印供电注入说明；每个对外连接器的脚位看板面丝印即可接线。

## 0. ⚠️ 当前终态（2026-07-01）：62×72mm、4 层——以本节为准

载板已**与外壳解耦、尺寸重置**（2026-06-29 reset-spec），并于 2026-07-01 完成最终重设计（commit `02d0e7d`）：

- **板框 = 62(X)×72(Y) mm，Ø88(R44) 四边切平圆角矩形**；**4 层**（L1 信号+器件 / L2 GND 平面 / L3 电源 / L4 信号），板厚 1.6mm。
- **布通 = Freerouting 自动布净 175/176 + 唯一手工网 `USB_CC2` 收尾**；**DRC = 0 violation / 0 unconnected / 0 short / 0 schematic-parity（62 项全 error、零 ignore）**。
- **中挖孔 = 15.5×50 封闭槽**（上下各留 5mm 桥；由需求让位 59 收入，装配前实板确认）；母座行距 17.8 / 节距 2.54 不变。
- 几何唯一真值 = [`../pcb/circuit/geom.py`](../pcb/circuit/geom.py) + 当前 `NanoSoul.kicad_pcb`。**下文 §2 机械表（94×92）与 §9（96.7×92.1 双层）均为历史版本记录。**
- ⚠️ **`../pcb/output/fab/` 现存 Gerber/CPL 是 2026-06-30 旧 2 层大板导出的，下单前必须按当前板重导**（见 §8）；BOM 已过嘉立创 SMT 核对可沿用（U6→QMI8658A `C3021082`、R9 shunt→1206 `C375525`）。

以下 reset-spec 冻结清单（电气/保护/连接器 KEEP 项）继续有效：
...
- **冻结几何（开发板锁定，非自由尺寸）**：母排行距 **17.8**/节距 2.54；中挖孔宽 **15.5**（长按 `geom.py` 真值：当前 **50** 封闭槽，需求让位上界 59）。
- **尺寸（自由，可靠 0/0 优先于小）**：板框 / 切平圆 / 白边 / 密度 / 孔位 / 摆位——全在 `geom.py`。历程：92×92 对新引脚过密布不干净 → 100×100 才 0/0（双层）→ **改 4 层后收敛到 62×72 终态**（自动布净 175/176，仅 USB_CC2 手工）。要再缩须重验布通。
...
> ⚠️ 下表是**重置前 92×92 旧板**的历史值（含安装孔坐标），仅留参考；**当前板 = 62×72、4 层（§0）**，机械真值见 `geom.py` / 当前 `NanoSoul.kicad_pcb`——装支架/外壳前从当前板文件重新导出孔位与连接器坐标，**勿用下表数字**。
...
3. 开发板底面元件 + 中部 FPC/microSD 落进**中部挖孔**（当前板 15.5×50 封闭槽）；USB-C（一端）与 C6 天线（另一端）悬在板外，**不要在其正下方放高件**。
4. 贴壳传感器（环境光 BH1750）走 **J8（JST-SH 4P）**引线，BH1750 可改贴外壳内壁。
5. ⚠️ 装板前用卡尺复核：开发板两排行距(~17.8mm)、挖孔（当前 15.5×50，确认盖住需下穿的元件段）、孔位——官方尺寸图未标全行距，以实板为准。
...
完整「开发板 40 脚 ↔ 功能信号」表见 [`BOARD_MAPPING.md`](BOARD_MAPPING.md)。要点：I²C0=IO7/8（板载音频共用），**I²C1 另两脚给外部传感器**；电机 PWM/方向、编码器、IMU SPI、电机电流 ADC 均从空闲 GPIO 引出。**该表已复核定稿（v1，2026-06-29：strapping / ADC / 外设矩阵均过）**，唯一遗留待办 = 眼校 Waveshare 原理图未引出脚。

## 7. BOM（载板，带 LCSC 号便于嘉立创打样贴片）

| 位号 | 器件 | 型号 | LCSC | 封装 |
|---|---|---|---|---|
| U6(IMU) | 6 轴 IMU | QMI8658C → **下单用 QMI8658A**（顶替 ICM-42688-P，4 线 SPI，脚位/驱动一致） | C2842151 → **C3021082** | LGA-14 |
...
> 嘉立创 SMT 核对（2026-06-30，已体现在 `output/NanoSoul_BOM.csv`）：U6 换同族 **QMI8658A `C3021082`**（C 版停产/邮寄，见 06 下单注意）；R9 电流采样 shunt 50mΩ 改 **1206（`C375525`）**。全单无留空/邮寄/停产料。

## 8. 制造（怎么下单）

- **板框**：外形为圆角矩形折线（Edge.Cuts）；**中部挖孔为铣槽（routed slot，当前 15.5×50 封闭槽）**，下单时确认厂家按内框铣穿。
- **孔**：安装孔均为 NPTH（非金属化），M3=3.2mm（≥4 个，孔位从当前板文件导出）。
- **规则**：铜到板边 ≥0.2mm；最小线宽/间距按厂家工艺（建议 ≥0.2mm）。
- **嘉立创/JLCPCB**：**4 层**、1.6mm、HASL/无铅；贴片用 §7 LCSC 号。
- ⚠️ **下单前必须重导 fab 包**：`pcb/output/fab/` 现存 Gerber / `NanoSoul_CPL.csv` 对应 2026-06-30 旧 2 层大板（fab README 写 ≈96.8×92.2，CPL 坐标超出当前 62×72 板框），**不可直接下单**。按当前 `NanoSoul.kicad_pcb` 重跑：`cd pcb/output/NanoSoul && ../../bin/kicad-cli pcb export gerbers . && ../../bin/kicad-cli pcb export drill .` + `fab_bom_cpl.py` 重出 BOM/CPL（BOM 料号可沿用）。

## 9. 布线状态（历史：2026-06-30 双层 96.7×92.1 版）

> ⚠️ 本节是 **96.7×92.1mm 双层板**的终态记录，已被 **2026-07-01 的 62×72、4 层重设计**（§0，commit `02d0e7d`）取代。方法学在 62×72 板上复用且依然有效：手布 USB_CC2 → `reroute_a_keep` → Freerouting best-of-N → `reroute_b_import` → `silk_clean`，加 §9.2 的 parity 归零脚本；这套流程与踩坑记录已沉淀进 `nanosoul-pcb` skill。

2026-06-30 终态（板框 **96.7×92.1mm**、双层、新引脚 = BOARD_MAPPING v1）：
...
- [ ] 母座行距 = 17.800mm、中挖孔宽 = 15.5（长按 `geom.py`，当前 50；冻结量未被缩动）
...
- [ ] IP5306 = I²C 版（C488349）；IMU 下单 = **QMI8658A（C3021082）**（C 版 C2842151 停产/邮寄，见 06）
- [ ] 引脚表 = `BOARD_MAPPING.md` **v1**（MOT_ISENSE=IO52 ADC2、MOTOR_STBY=IO33 带下拉、IMU_INT=IO23 原位）
- [ ] 卡尺复核：母座行距 17.8、挖孔 15.5×50（确认盖住下穿元件段）
